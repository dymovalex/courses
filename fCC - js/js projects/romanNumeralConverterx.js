function convertToRoman(num){
	...
	for (let i = num.length - 1, j = 0, k = 1; i >= 0; i--, j+=2, k++) {
		if((k == 4 && num[num.length - k] >= 4) || k > 4){
			j = 6;
			switch(num[num.length - k]){
		//	case "0": {answer = `${romanNumbers[j]}` + answer};
			case "9": {answer = `${romanNumbers[j]}` + answer};
			case "8": {answer = `${romanNumbers[j]}` + answer};
			case "7": {answer = `${romanNumbers[j]}` + answer};
			case "6": {answer = `${romanNumbers[j]}` + answer};
			case "5": {answer = `${romanNumbers[j]}` + answer};
			case "4": {answer = `${romanNumbers[j]}` + answer};
